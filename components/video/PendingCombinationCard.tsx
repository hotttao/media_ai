...
export function PendingCombinationCard({ item }: { item: PendingCombinationItem }) {
  ...
  return (
    <div className="flex min-w-0 max-w-sm flex-col overflow-hidden rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
      <div className="flex gap-3 p-3">
        <img src={item.firstFrame.url} alt={item.movement.content} className="h-16 w-14 rounded-lg object-cover shrink-0" />
      ...
      <div className="mt-auto flex items-center justify-between border-t border-white/5 px-3 py-1.5">
        <span className="truncate text-xs text-white/30">{item.combinationKey}</span>
        <button
          type="button"
          onClick={() => setShowNotice(true)}
          className="shrink-0 rounded-lg bg-matcha-600 px-3 py-1 text-xs font-medium text-white hover:bg-matcha-500"
        >
          发起生成
        </button>
      </div>
      {showNotice && (
        <p className="mx-3 mb-2 rounded-lg bg-black/20 px-3 py-1.5 text-xs text-white/60">

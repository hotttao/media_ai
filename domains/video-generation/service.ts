...
export async function generateEffectImage(
  ...
  const provider = providerRegistry.get('runninghub')
  if (!provider) {
    throw new Error('Provider runninghub not found')
  }
...
export async function generateVideo(
  ...
  // 5. 获取关联的 productMaterialId
  const productMaterial = await db.productMaterial.findFirst({
    where: {
      productId,
      ipId,
  ...
  // 6. 返回 videoId, videoUrl, productMaterialId
  return {
    videoId: video.id,
    videoUrl: video.url,
    productMaterialId: productMaterial?.id || '',

...
## 12. bug 修复
即梦生图:
1. 选择场景没有关联，虚拟IP场景的限定
2. 图片比例不对，调整为 9:16
3. 姿势现在只有文字没有图片
4. 生成组合内，展示每个组合的图片，如果已经生成展示生成的结果。每一行有个参数，点击可以复制提交job 的参数

review CombinationEngine 的代码，看看他们支持的哪些组合。代码有没有被正确复用。现在我想在首帧图的组合中添加一个平台(generation_path，数据库已经添加)。这是首帧图新增的唯一键。模特图和定妆图 没有。这个字段影响唯一性判断。

加入 generation_path，review 以下 即梦生图工具里面组合是否存在的判断逻辑，是否包含 generation_path 限定。即梦工具只跟 jimeng 平台的组合相关。

### 12.1 生图向导
定妆图:
1. 图片比例不对，调整为 9:16
2. 模特图应该放在上面，姿势应该放在下面

首帧图:
1. 缺少定妆图的选择
2. 缺少平台的选择
3. 组合里，没有显示结果，只显示了组合图。每一行有个参数按钮，点击后可以复制提交 job 的参数


## 13. 剪辑
当日发布计划-剪辑功能，大约要分为如下几个步骤:
1. 选择要剪辑的视频
2. 选择需要的背景音乐
3. 异步调用剪辑，调用本地的剪辑 cli 工具。

你需要添加一个不需要认证的回调接口，让 cli 工具调用，告知已经生成完成。

## 14. 每日计划
现在 daily_video_push 是不针对虚拟 IP，这个是不对的。但是现在的流程添加商品是从产品主页，不可能在添加的时候就指定 IP。所以我想的在查看当日发布计划的时候为每个 IP 指定发布计划。

指定方式有两种:
1. 从已经加入当日发布计划的产品中选择
2. 从已经发布的产品中选择

现在我还不清楚应该怎么设计 UI，怎么改 daily_video_push 表。我目前的想法是，当从商品主页将商品添加到当日发布计划时，给一个特殊标记的虚拟IP，比如叫 xxxxxxxxxxx。这个是固定的。你有没有更好的办法。


页面还有如下问题:
1. 未分配的状态显示不对，分配还是显示未分配
2. 分配IP 这个描述不对，应该叫加入。不应该有删除的概念，取消加入，可以在加入后，将加入变成取消
3. 最下方缺少添加商品的功能，从这里添加后的商品默认直接加入到当日发布计划中
4. 不要弹窗确认，很烦躁。直接在页面的每一行操作即可。
5. 还有个问题，加入我添加了新的商品，这个商品是不在当日产品发布计划中的。现在在虚拟IP的当日发布计划列表中会如何显示。正确行为应该是所有当日发布产品计划+用户自己选择的产品，即使用户没有选择所有当日发布计划中的产品，也需要展示，因为用户可能需要增加。这样方便用户操作。


## 15. 优化
1. 产品需要添加一个，是否带货标识，不能带货的，默认不会出现在所有生图生视频的组合中。



2. 在素材库里添加一个素材背景音乐，video_push 表中的 music_id 就是背景音乐的 id。
3. video_push 增加 title、content 字段，字符串表示发布标题和发布内容。
3. 优化页面 /daily-publish-plan/ip/9632369d-e1a0-4278-bff5-7d1540bb25ce?productId=3684738398762959071
  1. 剪辑成片我希望以表格形式展示，展示 video_push 如下字段:
    - 视频: 点击后可以播放剪辑完成的成片
    - music_id
    - template_name
    - thumbnail: 封面图
    ...
    - content: 发布内容
    - status: 视频状态
    - is_qualified: 这是一个选择框，我可以设置这个视频是否合格
    - is_published: 这是一个选择框，勾选后，标识发布，也可以取消发布
    - 按钮区域: AI 填充(通过AI 填充 thumbnail, title, content)，确认，标识状态更新成功提交数据库
  2. 删除顶部的 选择发布视频 添加商品
  3. 将剪辑、新增按钮，放入到 AI 生成视频


添加一个飞书推送的模块，参考实现在 D:\Code\media\MediaCrawler\core\notifier。
完成后，当用户点击发布后，通过飞书，把视频、标题、内容、封面，发送到飞书。
标题: 发布视频
内容: 视频标题
封面: 封面图
视频: 视频文件
